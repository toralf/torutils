...
function addTor() {
  ...
  do
    ...
    # rule 2
    $synpacket $hashlimit --hashlimit-srcmask 128 --hashlimit-htable-expire $(( 1*60*1000 )) --hashlimit-name tor-rule2-$orport --hashlimit-above 5/minute --hashlimit-burst 4 -j SET --add-set $blocklist src --exist
    $synpacket -m set --match-set $blocklist src -j DROP

    # rule 3
    $synpacket $hashlimit --hashlimit-srcmask 64 --hashlimit-htable-expire $(( 1*60*1000 )) --hashlimit-name tor-rule3-$orport --hashlimit-above 20/minute --hashlimit-burst 19 -j DROP

    # rule 4
    $synpacket $hashlimit --hashlimit-srcmask 128 --hashlimit-htable-expire $(( 2*60*1000 )) --hashlimit-name tor-rule4-$orport --hashlimit-above 30/hour --hashlimit-burst 1 -j DROP

    # rule 5
    $synpacket -m connlimit --connlimit-mask 128 --connlimit-above 4 -j SET --add-set $blocklist src --exist
    $synpacket -m set --match-set $blocklist src -j DROP
