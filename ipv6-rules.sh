...
function addTor() {
  ...
  for relay in $*; do
    ...
    # rule 3
    $common $hashlimit --hashlimit-name tor-ddos-$orport --hashlimit-above 10/minute --hashlimit-burst 1 --hashlimit-htable-expire $((2 * 60 * 1000)) -j SET --add-set $ddoslist src --exist
...
function saveCertainIpsets() {
  ipset list -n | grep -e 'tor-.*6-' -e 'tor-trust6$' |
